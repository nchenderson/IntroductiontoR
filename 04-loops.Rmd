...
* [**Loops**]{style="color:#D96716"} are typically used when we want to repeat some type 
of calculation many times.

* The two types of loops in **R** are the [**for**]{style="color:#D96716"} loop 
and the [**while**]{style="color:#D96716"} loop.

* [**For**]{style="color:#D96716"} loops: 
    + Typically used when we know exactly how many times we need to repeat a calculation.
    
* [**While**]{style="color:#D96716"} loops:
...
* The [**for**]{style="color:#D96716"} loop will execute the code underneath the [**for**]{style="color:#D96716"} statement $T$ times where $T$ is the **length** of the vector `vec_name`.
...
* As an example, let's write a [**for**]{style="color:#D96716"} loop that prints the first 5 positive integers:
```{r, echo=TRUE}
for (k in 1:5) {
    print(k)
}
...
* [**Each time**]{style="color:#D96716"} you go inside the loop, the variable `k` takes the next value in the vector `1:5` and is printed to the screen.


### Example 2: computing 10 factorial

* As another example, let's use a [**for**]{style="color:#D96716"} loop to compute the product
\begin{equation}
...
* Here is an example where the looping variable is a [**character**]{style="color:#D96716"}.
```{r, echo = TRUE}
...
* The [**Fibonacci sequence**]{style="color:#D96716"} is a sequence of numbers $x_{1}, x_{2}, x_{3}, ...$ defined by the relationship:
\begin{equation}
x_{k} = x_{k-1} + x_{k-2} \nonumber
\end{equation}
...
* You can have a for loop **within** another [**for**]{style="color:#D96716"} loop. 
...
### Exiting for loops and skipping iterations: break and next
...
* [**For**]{style="color:#D96716"} loops execute a piece of code a set number of times.

* [**While**]{style="color:#D96716"} loops keep executing the piece of code as long as 
a certain condition is `TRUE`.

* The general form of a [**while**]{style="color:#D96716"} loop in **R** is
```{r, echo=TRUE, eval=FALSE}
while(condition) {
    code chunk
...
* `condition` is a [**logical**]{style="color:#D96716"} expression.

* The `code_chunk` inside the loop is repeated until the `condition` becomes `FALSE`
in which case the loop [**stops**]{style="color:#D96716"}. 
...
* We can use a [**while loop**]{style="color:#D96716"} to find the [**first occurrence**]{style="color:#D96716"} of a number in a vector.
